...
function mapDataToContacts(rawData) {
  ...
  const headerMap = {
    // prenom
    prenom: 'prenom', firstname: 'prenom', first: 'prenom', firstnamee: 'prenom', firstnam: 'prenom',
    // nom
    nom: 'nom', lastname: 'nom', last: 'nom', surname: 'nom',
    // email
    email: 'email', eemail: 'email', mail: 'email', courriel: 'email',
    // telephone
    telephone: 'telephone', phone: 'telephone', mobile: 'telephone', tel: 'telephone', gsm: 'telephone',
    // sexe
    sexe: 'sexe', sex: 'sexe', gender: 'sexe',
    // statut
    statut: 'statut', status: 'statut',
    // ville
    ville: 'ville', city: 'ville',
    // entreprise
    entreprise: 'entreprise', company: 'entreprise', societe: 'entreprise',
    // categorie
  ...
  if (rawData.length > 0) {
    ...
    if (!hasEmail) {
      throw new Error(`Invalid file format. No email column detected. Found: ${availableColumns.join(', ')}`);
    }
  }
    ...
    for (const nk of Object.keys(normalizedRow)) {
      ...
      switch (dbCol) {
          ...
          case 'tags':
            ...
            contact._tags = value.split(',').map(tag => tag.trim()).filter(tag => tag);
            break;
          case 'segments':
            // Split segments by comma and store for later processing
            contact._segments = value.split(',').map(seg => seg.trim()).filter(seg => seg);
            break;
      }
    }
  ...
  console.log(`Processed ${processedContacts.length} contacts from ${rawData.length} rows (${rawData.length - processedContacts.length} duplicates/skipped)`);
