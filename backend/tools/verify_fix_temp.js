...
const { buildContactQueryFromCriteria } = require(
  path.join(rootPath, 'backend/controllers/segmentController.js')
);
...
async function verify() {
  ...
  try {
    ...
    if (
      whereInspect.includes('SELECT contact_id FROM contact_tag') &&
      whereInspect.includes('1549')
    ) {
