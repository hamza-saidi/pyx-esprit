...
async function auditCampaign(campaignId) {
  try {
    ...
    const totalOpenRows = await EnvoiEmail.count({
      where: { campagne_id: campaignId, date_ouverture: { [Op.not]: null } },
    });
    const totalClickRows = await EnvoiEmail.count({
      where: { campagne_id: campaignId, date_clic: { [Op.not]: null } },
    });
    ...
    if (totalRows < stats.nb_envoyes) {
      console.log(
        '\n⚠️ ATTENTION: Il manque ' +
          (stats.nb_envoyes - totalRows) +
          ' lignes dans EnvoiEmail par rapport au total déclaré.'
      );
    }
    ...
    if (rowsWithContact < totalRows) {
      console.log(
        '\n⚠️ ATTENTION: ' +
          (totalRows - rowsWithContact) +
          " lignes n'ont pas de contact valide (jointure impossible)."
      );
    }
  } catch (error) {
