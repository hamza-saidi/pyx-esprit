...
const SOURCE_TAG_NAMES = ['TO GOLF Belgique', 'TO GOLF BELGE'];
  ...
  try {
    ...
    for (const contact of contactsWithSourceTags) {
      ...
      if (!contactTagIds.includes(targetId)) {
        if (DRY_RUN) {
          console.log(
            `[DRY] + Ajouter tag "${TARGET_TAG_NAME}" au contact ${contact.id} (${contact.prenom} ${contact.nom})`
          );
        } else {
    ...
    if (DELETE_OLD_TAGS && !DRY_RUN) {
      ...
      for (const sourceTag of sourceTags) {
        ...
        if (remainingContacts > 0) {
          console.log(
            `⚠ Le tag "${sourceTag.nom}" est encore utilisé par ${remainingContacts} contact(s), pas de suppression`
          );
        } else {
          try {
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(
      DRY_RUN ? '✓ SIMULATION TERMINÉE (aucune modification réelle)' : '✓ FUSION TERMINÉE'
    );
    console.log(`${'='.repeat(60)}\n`);
  } catch (error) {
