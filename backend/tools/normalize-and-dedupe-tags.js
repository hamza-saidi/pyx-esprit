...
async function main() {
  ...
  try {
    ...
    for (const { duplicate, primary } of duplicates) {
      ...
      const [result] = await sequelize.query('UPDATE contact_tag SET tag_id = ? WHERE tag_id = ?', {
        replacements: [primary.id, duplicate.id],
        transaction: t,
      });
    ...
    await t.commit();
    console.log(
      `Done. Renamed tags: ${renamed}, Duplicates merged: ${deletedTags}, Links reassociated: ${reassociatedLinks}`
    );
  } catch (err) {
    await t.rollback();
