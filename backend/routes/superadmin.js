  ...
  try {
    ...
    const result = await Promise.all(clubs.map(async (club) => {
      const [contacts, users, campagnes] = await Promise.all([
        runWithTenant({ clubId: club.id, isSystem: true }, () => db.Contact.count({ where: { club_id: club.id } })),
        runWithTenant({ clubId: club.id, isSystem: true }, () => db.Utilisateur.count({ where: { club_id: club.id } })),
        runWithTenant({ clubId: club.id, isSystem: true }, () => db.CampagneEmail.count({ where: { club_id: club.id } })),
      ]);
...
router.post('/clubs', async (req, res, next) => {
  try {
    ...
    if (!nom || !slug || !admin_email || !admin_password)
      return res.status(400).json({ message: 'nom, slug, admin_email et admin_password sont requis.' });
    ...
    const club = await db.Club.create({ nom, slug, email_contact: email_contact || admin_email, statut: 'actif' });

    const hash = await bcrypt.hash(admin_password, 12);
    await runWithTenant({ clubId: club.id, isSystem: false }, () =>
      db.Utilisateur.create({ nom: admin_nom || nom, email: admin_email, mot_de_passe: hash, role: 'admin', club_id: club.id })
    );

    res.status(201).json({ message: `Club "${nom}" créé avec succès.`, club_id: club.id, slug: club.slug });
  } catch (err) { next(err); }
});
...
router.get('/stats', async (req, res, next) => {
  try {
    ...
    let totalContacts = 0, totalCampagnes = 0, totalEnvois = 0;
    for (const club of clubs) {
      totalContacts  += await runWithTenant({ clubId: club.id, isSystem: true }, () => db.Contact.count({ where: { club_id: club.id } }));
      totalCampagnes += await runWithTenant({ clubId: club.id, isSystem: true }, () => db.CampagneEmail.count({ where: { club_id: club.id } }));
      totalEnvois    += await runWithTenant({ clubId: club.id, isSystem: true }, () => db.EnvoiEmail.count({ where: { club_id: club.id } }));
    }
    res.json({
      clubs: clubs.length,
