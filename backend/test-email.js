...
async function testEmailService() {
  ...
  try {
    ...
    console.log('   Variables remplacées:', contenuPersonnalise.includes('{{email}}') ? '❌' : '✅');
    console.log('   Token remplacé:', contenuPersonnalise.includes('{{tracking_token}}') ? '❌' : '✅');
    console.log('   Lien désabonnement:', contenuPersonnalise.includes('{{unsubscribe_link}}') ? '❌' : '✅');
    console.log('   Pixel tracking:', contenuPersonnalise.includes('{{tracking_pixel}}') ? '❌' : '✅\n');

    // 5. Test de conversion HTML vers texte
    console.log('5. Test de conversion HTML vers texte...');
    const htmlTest = '<h1>Titre</h1><p>Paragraphe avec <strong>gras</strong> et <em>italique</em>.</p>';
