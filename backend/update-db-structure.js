...
async function updateDatabaseStructure() {
  ...
  try {
    ...
    for (const update of templateUpdates) {
      try { await sequelize.query(update); console.log('   ✅ Structure modèle mise à jour'); }
      catch (error) {
        if (error.message.includes('Duplicate column name')) console.log('   ℹ️ Colonne déjà présente');
