...
exports.getByCampaign = async (req, res) => {
  try {
    ...
    console.log(`[DEBUG] getByCampaign request: status="${status}", search="${search}", page=${page}`);
    ...
    console.log(`[DEBUG] Campagne ${campaignId}: ${totalCount} envois totaux, ${envois.length} sur cette page (status: ${status})`);
    ...
    const timelineData = await EnvoiEmail.findAll({
      ...
      attributes: [
        ...
        [sequelize.fn('SUM', sequelize.literal('CASE WHEN date_ouverture IS NOT NULL THEN 1 ELSE 0 END')), 'ouverts'],
        [sequelize.fn('SUM', sequelize.literal('CASE WHEN date_clic IS NOT NULL THEN 1 ELSE 0 END')), 'clics']
      ],
    ...
    const [openersCount, clickersCount, errorsCount] = await Promise.all([
      ...
      EnvoiEmail.count({ where: { campagne_id: campaignId, statut: { [Op.in]: ['erreur', 'bounce', 'spam'] } } })
    ]);
  ...
  try {
    ...
    const statsEnvois = await EnvoiEmail.findAll({
      ...
      attributes: [
        'statut',
        [sequelize.fn('COUNT', sequelize.col('id')), 'count'],
        [sequelize.fn('COUNT', sequelize.fn('DISTINCT', sequelize.col('campagne_id'))), 'campagnes_uniques']
      ],
      group: ['statut']
    });
    ...
    const statsEvenements = await Evenement.findAll({
      ...
      attributes: [
        [sequelize.fn('COUNT', sequelize.col('id')), 'total_evenements']
      ]
    });
    ...
    const totalCampagnes = statsCampagnes.reduce((sum, stat) => sum + parseInt(stat.dataValues.count), 0);
    const totalEnvois = statsEnvois.reduce((sum, stat) => sum + parseInt(stat.dataValues.count), 0);
    const totalContacts = statsContacts.reduce((sum, stat) => sum + parseInt(stat.dataValues.count), 0);
    const totalEvenements = statsEvenements.length > 0 ? parseInt(statsEvenements[0].dataValues.total_evenements) : 0;
    ...
    growthDataRaw.forEach(row => {
        const dStr = row.date;
        const dateStr = dStr instanceof Date ? dStr.toISOString().split('T')[0] : 
                        (typeof dStr === 'string' && dStr.length >= 10 ? dStr.substring(0, 10) : dStr);
...
exports.getStatsBySegment = async (req, res) => {
  try {
    ...
    if (statsSegment.total_envois > 0) {
      statsSegment.taux_ouverture = (statsSegment.total_ouverts / statsSegment.total_envois * 100).toFixed(2);
      statsSegment.taux_clic = (statsSegment.total_clics / statsSegment.total_envois * 100).toFixed(2);
    }
    ...
    const performanceParCampagne = campagnes.map(campagne => ({
      ...
      taux_ouverture: campagne.statistiques?.nb_envoyes > 0 ? 
        (campagne.statistiques.nb_ouverts / campagne.statistiques.nb_envoyes * 100).toFixed(2) : 0,
      taux_clic: campagne.statistiques?.nb_envoyes > 0 ? 
        (campagne.statistiques.nb_clics / campagne.statistiques.nb_envoyes * 100).toFixed(2) : 0
    }));
...
exports.getEventStats = async (req, res) => {
  try {
    ...
    const statsEvenements = {
      ...
      total_confirmes: evenements.reduce((sum, e) => 
        sum + e.rsvps.filter(r => r.statut === 'confirmé').length, 0),
      total_absents: evenements.reduce((sum, e) => 
        sum + e.rsvps.filter(r => r.statut === 'absent').length, 0),
      taux_confirmation: 0,
      taux_participation: 0
    };

    if (statsEvenements.total_invites > 0) {
      statsEvenements.taux_confirmation = (statsEvenements.total_confirmes / statsEvenements.total_invites * 100).toFixed(2);
      statsEvenements.taux_participation = ((statsEvenements.total_confirmes + statsEvenements.total_absents) / statsEvenements.total_invites * 100).toFixed(2);
    }
    ...
    const performanceParEvenement = evenements.map(evenement => {
      ...
      return {
        ...
        taux_participation: totalRsvp > 0 ? ((confirmes + absents) / totalRsvp * 100).toFixed(2) : 0
      };
    });
