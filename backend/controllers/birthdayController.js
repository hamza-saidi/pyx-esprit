...
exports.listToday = async (req, res) => {
  try {
    ...
    const contacts = await Contact.findAll({
      where: where(fn('DATE_FORMAT', col('date_naissance'), '%m-%d'), `${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`),
...
exports.sendToday = async (req, res) => {
  try {
    ...
    let sent = 0; let errors = 0; const results = [];
    for (const c of contacts) {
      try {
        ...
        const campagneShim = { id: `bday-${today.toISOString().slice(0,10)}`, sujet, contenu_html, contenu_texte };
        ...
        sent++; results.push({ id: c.id, email: c.email, status: 'sent' });
        ...
        errors++; results.push({ id: c.id, email: c.email, status: 'error', error: e.message });
      }
    }
