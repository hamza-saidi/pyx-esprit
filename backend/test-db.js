...
async function testDatabase() {
  ...
  try {
    ...
    console.log('\n📋 Données existantes:');
    const [campagnes] = await sequelize.query('SELECT id, titre, statut, type_campagne, date_creation FROM campagne_email LIMIT 5');
    campagnes.forEach(camp => {
      console.log(`   - ID: ${camp.id}, Titre: ${camp.titre}, Statut: ${camp.statut}, Type: ${camp.type_campagne}`);
    });
